...
public class StatsDClientTest extends TimerTask {

    private ByteBuffer sendBuffer;
    private Timer flushTimer;
    private boolean multi_metrics = false;
    private static final Random RNG = new Random();
    private static final Logger log = Logger.getLogger(StatsDClientTest.class.getName());
    private final InetSocketAddress _address;
    private final DatagramChannel _channel;
    public StatsDClientTest(String host, int port) throws UnknownHostException, IOException {
        this(InetAddress.getByName(host), port);
    }
    public StatsDClientTest(InetAddress host, int port) throws IOException {
    ...
    public static void main(String[] arg){
        while(true){
            ...
            try {
                StatsDClientTest client = new StatsDClientTest("127.0.0.1", 8125);
                //client.increment("statsd.login.error");
                client.increment("statsd.access.error", x1);
                client.gauge("statsd.user", x2);
